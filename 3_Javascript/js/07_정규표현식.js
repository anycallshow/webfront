...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    const regExp1 = /a/;
    div.innerHTML += "/a/ , apple : " + regExp1.test("apple") + "<hr>"; // true
    div.innerHTML += "/a/ , apple : " + regExp1.test("price") + "<hr>"; // false

    // [abcd] : 문자열 내에 a,b,c,d 중에 하나라도 일치하는 문자가 있는지 검색
    const regExp2 = /[abcd]/;
    div.innerHTML += "/[abcd]/ , apple : " + regExp2.test("apple") + "<hr>"; // true 
    div.innerHTML += "/[abcd]/ , sub : " + regExp2.test("sub") + "<hr>"; // true 
    div.innerHTML += "/[abcd]/ , qwer : " + regExp2.test("qwer") + "<hr>"; // false

    // ^(캐럿) : 문자열의 시작을 의미
    const regExp3 = /^group/; // 문자열의 시작이 "group"인지 확인
    div.innerHTML += "/^group/ , group100 : " + regExp3.test("group100") + "<hr>"; // true
    div.innerHTML += "/^group/ , 100group : " + regExp3.test("100group") + "<hr>"; // false

    // $(달러) : 문자열의 끝을 의미
    const regExp4 = /group$/; // 문자열의 끝이 "group"인지 확인
    div.innerHTML += "/group$/ , group100 : " + regExp4.test("group100") + "<hr>"; // false
    div.innerHTML += "/group$/ , 100group : " + regExp4.test("100group") + "<hr>"; // true
...
document.getElementById("inputName").addEventListener("keyup", function(){
    ...
    if(regExp.test(this.value)){
        ...
        // -> 유효한 이름 형식입니다. (초록글씨 진하게)
        span.innerHTML = "유효한 이름 형식입니다.";
        span.style.color = "green";
        span.style.fontWeight = "bold";

    } else { // 아닌 경우
             // 이름 형식이 유효하지 않습니다. (빨간글씨 진하게)
...
document.getElementById("inputPno").addEventListener("keyup",function(){
    ...
    if(regExp.test(this.value)){
        ...
        span.innerHTML = "올바르지 않은 주민등록번호입니다."
